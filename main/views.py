...
def home(request):
    ...
    trending = Trending.objects.all() ### Trending now will show on top bar(send query from here to naster.html in front)
...
def category_view(request, category_name):
    # Fetch news articles that match the given category name
    news_articles = News.objects.filter(catname=category_name, act=1)  # Assuming 'act=1' means published news
    site = Main.objects.first()
    news = News.objects.filter(act=1).order_by('-pk')  ## for reverse(ordering) need to filter by pk with (-) to get the latest submission first.
        ...
        if utxt != "" and ptxt != "":
            ...
            if user != None:
                try:
                    ...
                    messages.error(request,"User is found but your are not allowed as staff Please contact with Administrator to get access to this site.")

            else:
                messages.error(request,"Invalid Username or Password, Please go back to login or create new account")
        elif utxt == '' or ptxt == '':
            messages.error(request, 'Inputs are required please fill them all')
...
def myregister(request):
    ...
    if request.method == 'POST':
        ...
        if name == "" :
            messages.error(request,"Input Your Name")
