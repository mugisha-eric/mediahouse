...
def news_detail(request,word):
    ...
    code = News.objects.get(name=word).pk  ## For Comment Section
    news_article = News.objects.get(name=word)
    prk = news_article.pk

    comments = Comment.objects.filter(news_id=prk)    # To show comment

    cmcount = len(comments)  # When there is comment, then show the comment section
        ...
        try:
            ...
            if str(myfile.content_type).startswith("image"):
                ...
                if myfile.size < 5000000:## Check File Size ##
                    ...
                    b = News(name=newstitle, short_txt=newstxtshort, body_txt=newstxt, date=today, picname=filename, picurl=url, writer=request.user, catname=newsname, catid=newsid, show=1, act=1, time=time, ocatid=ocatid, tag=tag, rand=rand)  # these are the model fields
                    b.save()
